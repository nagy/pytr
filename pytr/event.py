...
class Event:
    ...
    def _parse_shares_and_fees(cls, event_dict: Dict[Any, Any]) -> Tuple[Optional[float], Optional[float]]:
        ...
        sections = event_dict.get("details", {}).get("sections", [{}])
        overview_datas = next(iter(filter(lambda x: x.get("title") in ["Übersicht"], sections)), {}).get("data", [])
        transaction_dicts = filter(lambda x: x.get("title") in ["Transaktion"], overview_datas)
